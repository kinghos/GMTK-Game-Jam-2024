...
var resize_queue: Array = []
var resize_tween: Tween = null

func _ready():
	$CollisionShape2D.scale = Vector2(0.2, 0.2)
	
func resize(new_size: Vector2, new_mass: float, new_friction: float, resize_duration: float):
	...
	resize_tween.tween_property($CollisionShape2D, "scale", new_size, resize_duration)
	resize_tween.tween_property($".", "mass", new_mass, resize_duration)
